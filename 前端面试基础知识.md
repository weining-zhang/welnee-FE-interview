...
- **4）页面解析处理阶段**
  ...
  - **JS**
    ...
    - 异步加载js脚本（defer 和 async
    - 根据路由动态加载模块
    - 减小打包体积
      1. gzip 压缩
      2. Tree Shaking
    - webpack 缓存，[webpack-缓存](https://alienzhou.github.io/fe-performance-journey/5-subresources/javascript.html#_4-%E7%BC%93%E5%AD%98)
      1. 将基础库代码合并单独打包，使用webpack的 [`optimization.splitChunks`](https://webpack.docschina.org/plugins/split-chunks-plugin/#optimizationsplitchunks) 配置
      2. 通过 [`optimization.runtimeChunk`](https://webpack.js.org/configuration/optimization/#optimizationruntimechunk) 配置将 runtime chunk 单独拆分出来
  ...
  - **CSS**
    ...
    - 减少选择器嵌套，不要超过三层
    - webpack 缓存
      - 使用 [MiniCssExtractPlugin](https://webpack.js.org/plugins/mini-css-extract-plugin/) 插件将样式信息单独抽离出 CSS 文件
